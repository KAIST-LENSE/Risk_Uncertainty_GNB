...
% (b) Generate Combined Bayesian Classification Matrix
classmat_comb = zeros(n_sim,1);
parfor i = 1:n_sim
    if fx(i,1) <= targets(1) && fx(i,2) < targets(2)
        classmat_comb(i) = 1;
    elseif fx(i,2) > targets(1) && fx(i,2) >= targets(2)
        classmat_comb(i) = 0;
    else
        classmat_comb(i) = 2;   % For data that only meets 1 of the conditions
    end
end
%%Progress Bar%%
...
Success_Mat = zeros(n_sim, n_kp);
Failure_Mat = zeros(n_sim, n_kp);
% (b) Loop through "classmat_comb" then append values from ParamSpace to 
%     Success_Mat or Failure_Mat accordingly
parfor i = 1:n_sim
    if classmat_comb(i) == 1
        Success_Mat(i,:) = ParSpace(i,:);
        Failure_Mat(i,:) = zeros(1,n_kp,'uint32');
    elseif classmat_comb(i) == 0
        Success_Mat(i,:) = zeros(1,n_kp,'uint32');
        Failure_Mat(i,:) = ParSpace(i,:);
    else
        Success_Mat(i,:) = ParSpace(i,:);
        Failure_Mat(i,:) = ParSpace(i,:);
    end
...
Stake_Risk_Score = zeros(n_kp,1);         % Normalized
...
parfor i = 1:n_kp
    ...
    Stake_Risk_Score(i)  = trapz(X_LIST, Posterior_Diff);
    ...
    title(['For Parameter ', num2str(i), '- Combined'])
    legend('Success Posterior', 'Failure Posterior', 'Posterior Prob. Diff.')
    hold off
    saveas(gcf, ['POSTERIOR_nCaCO3_merged_parameter_',num2str(i),'.png'])
end
...
fprintf('Stakeholders Risk Score for All Criterias is')
Stake_Risk_Score
waitbar(990/total_steps, progress, 'Plotting Distributions of System Outputs');
...
writecell( BioAdditive_Res, 'BioAdditive_Results_Merged.csv');
